...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        openNextImg = action('&Next Image', self.openNextImg,
                            #  'd', 'next', u'Open Next')
                            'Ctrl+N', 'next', u'Open Next')
        propagateToNextImg = action('&Propagate to Next Image', self.propagateToNextImg,
                            'Ctrl+Shift+N', 'propagate next', u'Propagate to Next')

        openPrevImg = action('&Prev Image', self.openPrevImg,
                             'Ctrl+P', 'prev', u'Open Prev')

        propagateToPrevImg = action('&Propagate to Prev Image', self.openPrevImg,
                             'Ctrl+Shift+P', 'propagate prev', u'Propagate to Prev')
        ...
        zoomOrg = action('&Original size', partial(self.setZoom, 100),
                         'Ctrl+0', 'zoom', u'Zoom to original size', enabled=False)
        ...
        self.paintLabelsOption.setShortcut("Ctrl+Alt+P")
        ...
        self.actions.advanced = (
            ...
            hideAll, showAll, None, propagateToNextImg, propagateToPrevImg)
    ...
    def loadFile(self, filePath=None, propagate_bbox=False):
        ...
        if unicodeFilePath and os.path.exists(unicodeFilePath):
            ...
            if self.defaultSaveDir is not None:
                ...
                elif (self.propBoundingBox.isChecked() or propagate_bbox) and self.prevFileName is not None: # Ravindra --> Use this below condition if atleast one file has been read previously to be extended for the next frame
                # Ravindra --> read the previously saved xml file and carry it forward to the new image if xmlPath is not found
    ...
    def openPrevImg(self, _value=False, propagate_bbox=False):
        ...
        if currIndex - 1 >= 0:
            ...
            if filename:
                self.loadFile(filename, propagate_bbox=propagate_bbox)

    def propagateToPrevImg(self, _value=False):
        self.openPrevImg(_value, propagate_bbox=True)

    def openNextImg(self, _value=False, propagate_bbox=False):
        # Proceding prev image without dialog if having any label
        if self.autoSaving.isChecked():
            if self.defaultSaveDir is not None:
                if self.dirty is True:
    ...
    def propagateToNextImg(self, _value=False):
        self.openNextImg(_value, propagate_bbox=True)
